...
If the extension shows **CLI not detected** after install, rerun the one-command companion installer, fully close and reopen Chrome, and then click **Refresh** in the extension. The current Windows companion saves the installer PATH so Chrome native messaging can find the same `sf` command that works in Command Prompt or PowerShell. It detects Salesforce CLI with `where`, but runs validation through `sf --version` instead of an absolute npm shim path, avoiding quoted `.cmd` failures such as `'"C:\Users\...\npm\sf.cmd"' is not recognized` and `'"C:\Program Files\sf\bin\sf.cmd"' is not recognized`.
