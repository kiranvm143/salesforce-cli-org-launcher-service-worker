...
On Windows, the refreshed packages also detect Salesforce CLI through `where`, save the install-time PATH for Chrome native messaging, and validate with `sf --version` through `PATH` instead of calling an absolute npm shim path. This fixes cases where `sf --version` works in Command Prompt but Chrome's native host resolved a quoted `.cmd` shim such as `C:\Users\...\AppData\Roaming\npm\sf.cmd` or `C:\Program Files\sf\bin\sf.cmd` and failed with a “not recognized” error.
